...
function Grid(db, mongo) {
  ...
  this.db = db;
  this.mongo = mongo;
  this.curCol = this.mongo.GridStore
    ? this.mongo.GridStore.DEFAULT_ROOT_COLLECTION
    : "fs";
}
...
Grid.prototype.collection = function(name) {
  this.curCol =
    name || this.curCol || this.mongo.GridStore.DEFAULT_ROOT_COLLECTION;
...
Grid.prototype.findOne = function(options, callback) {
  ...
  var collection =
    options.root && options.root != this.curCol
      ? this.db.collection(options.root + ".files")
      : this.files;
  if (!collection) {
